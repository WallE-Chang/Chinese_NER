...
class CRF(nn.Module):
    ...
    def _compute_log_partition(self, emissions, mask):
        ...
        for i in range(1, seq_length):
            ...
            scores = e_scores + t_scores + a_scores #(bs, nb_labels, nb_labels)

            #(bs, nb_labels, nb_labels)  ->  (bs, nb_labels)
            new_alphas = torch.logsumexp(scores, dim=1) 

            # set alphas if the mask is valid, otherwise keep the current values
            is_valid = mask[:, i].unsqueeze(-1)  #(bs,)
            alphas = is_valid * new_alphas + (1 - is_valid) * alphas #(bs, nb_labels)

        # add the scores for the final transition
        last_transition = self.transitions[:, self.EOS_TAG_ID]
        end_scores = alphas + last_transition.unsqueeze(0) #(bs, nb_labels)
    ...
    def _viterbi_decode(self, emissions, mask):
        ...
        for i in range(1, seq_length):
            ...
            max_scores, max_score_tags = torch.max(scores, dim=1) #(bs, nb_labels)
            ...
            backpointers.append(max_score_tags.t()) #(seq_length,nb_labels,bs)
